...
var BuildRetention = require("./buildRetention");
var resources = require("../resources/resourceManager");
var utils = require("taco-utils");
var BuildInfo = utils.BuildInfo;
var Logger = utils.Logger;
;
var BuildManager = (function () {
    function BuildManager(conf) {
    ...
    BuildManager.prototype.shutdown = function () {
        if (this.deleteBuildsOnShutdown) {
            this.buildRetention.deleteAllSync(this.builds);
        }
    };
    BuildManager.prototype.submitNewBuild = function (req) {
        ...
        var buildPlatform = req.query.platform || "ios";
        var logLevel = req.query.logLevel || null;
        var self = this;
        return this.requestRedirector.getPackageToServeRequest(req).then(function (pkg) {
            ...
            self.saveUploadedTgzFile(buildInfo, req, function (err, result) {
                if (err) {
                    deferred.reject(err);
                }
                else {
    ...
    BuildManager.prototype.getBuildInfo = function (id) {
        return this.builds[id] || null;
    };
    BuildManager.prototype.downloadBuildLog = function (id, offset, res) {
        var buildInfo = this.builds[id];
        if (!buildInfo) {
            res.end();
            return;
        }
        var buildLog = path.join(buildInfo.buildDir, BuildManager.BUILD_LOG_FILE);
    ...
    BuildManager.prototype.getAllBuildInfo = function () {
        return {
    ...
    BuildManager.prototype.downloadBuild = function (buildInfo, req, res) {
        var self = this;
        if (!buildInfo["pkg"]) {
            res.status(404).send(resources.getStringForLanguage(req, "MalformedBuildInfo"));
            return;
        }
        buildInfo["pkg"].downloadBuild(buildInfo, req, res, function (err) {
            if (!err) {
    ...
    BuildManager.prototype.getBaseBuildDir = function () {
        return this.baseBuildDir;
    };
    BuildManager.prototype.emulateBuild = function (buildInfo, req, res) {
    ...
    BuildManager.prototype.deployBuild = function (buildInfo, req, res) {
        if (!buildInfo["pkg"]) {
            res.status(404).send(resources.getStringForLanguage(req, "MalformedBuildInfo"));
            return;
        }
    ...
    BuildManager.prototype.runBuild = function (buildInfo, req, res) {
        if (!buildInfo["pkg"]) {
            res.status(404).send(resources.getStringForLanguage(req, "MalformedBuildInfo"));
            return;
        }
    ...
    BuildManager.prototype.debugBuild = function (buildInfo, req, res) {
        if (!buildInfo["pkg"]) {
            res.status(404).send(resources.getStringForLanguage(req, "MalformedBuildInfo"));
            return;
        }
    ...
    BuildManager.prototype.saveUploadedTgzFile = function (buildInfo, req, callback) {
        var self = this;
    ...
    BuildManager.prototype.beginBuild = function (req, buildInfo) {
        var self = this;
        ...
        var pluginsOnlyFilter = function (who) {
            ...
            var localPath = path.relative(extractToRemoteDir, who.props.path);
        ...
        // TODO: Remove the casting once we've get some complete/up-to-date .d.ts files. See https://github.com/Microsoft/TACO/issues/18
        var pluginExtractor = tar.Extract({ path: extractToRemoteDir, strip: 1, filter: pluginsOnlyFilter });
        pluginExtractor.on("end", function () {
        ...
        Q.all([extractDeferred.promise, extractPluginDeferred.promise]).then(function () {
    ...
    BuildManager.prototype.removeDeletedFiles = function (buildInfo) {
        ...
        if (fs.existsSync(changeListFile)) {
            ...
            if (buildInfo.changeList) {
                buildInfo.changeList.deletedFiles = buildInfo.changeList.deletedFiles.map(function (deletedFile) {
                ...
                buildInfo.changeList.deletedFiles.forEach(function (deletedFile) {
                    if (deletedFile.split(path.sep)[0] !== "plugins") {
                        ...
                        var fileToDelete = path.resolve(buildInfo.appDir, deletedFile);
    ...
    BuildManager.prototype.build = function (buildInfo) {
        var self = this;
        ...
        if (!buildInfo["pkg"]) {
            ...
            buildInfo["pkg"].build(buildInfo, function (resultBuildInfo) {
                ...
                utils.TelemetryHelper.generate("build", function (telemetry) {
                    telemetry
    ...
    BuildManager.prototype.dequeueNextBuild = function () {
    ...
    BuildManager.BUILD_LOG_FILE = "build.log";
    return BuildManager;
}());
