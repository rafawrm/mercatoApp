...
var tacoError = require("../tacoError");
describe("taco Errors in taco-utils", function () {
    it("Verify taco Errors in taco-utils", function () {
    ...
    it("getError() should return an error with the correct message for errors that have no placeholders", function () {
        var testResourceManager = new resourceManager.ResourceManager(path.join(__dirname, "resources"), "en");
    ...
    it("wrapError() should return an error and inner error with the correct messages for errors that have no placeholders", function () {
        var innerErrorString = "Inner error";
        var testResourceManager = new resourceManager.ResourceManager(path.join(__dirname, "resources"), "en");
        var innerError = new Error(innerErrorString);
        var testError = tacoError.TacoError.wrapError(innerError, "SimpleMessage", 10, testResourceManager);
        should(testError.message).equal(testResourceManager.getString("SimpleMessage"));
        should(testError.innerError.message).equal(innerErrorString);
    });
});
...
function verifyTacoErrors(fileName, resources, minErrorCode, maxErrorCode) {
    var errorCodes = require(fileName);
    ...
    Object.keys(errorCodes).forEach(function (errorCode) {
        ...
        if (isNaN(parseInt(errorCode, 10))) {
            ...
            var numericErrorCode = errorCodes[errorCode];
            errorCode.should.be.equal(errorCodes[numericErrorCode], errorCode + " and " + errorCodes[numericErrorCode] + " have been assigned same error code");
...
//# sourceMappingURL=tacoErrors.js.map
